...
def ingest_novel_knowledge(system, train_df, test_mode=True):
    ...
    if os.path.exists(cache_path) and not test_mode:
        ...
        try:
            # TRUSTED SOURCE: Loading local cache generated by this same system
    ...
    for _, row in distinct_chars.iterrows():
        ...
        if book in system.world_states:
            # FIX for >85%: Higher focus on character-specific state (alpha=0.2)
            system.backstory_states[key] = system.world_states[book].get_query_state(char, alpha=0.2)
        else:
...
def _entity_aware_ingest(system, text: str, entities: set) -> WorldState:
    # ULTRA-SPEED MODE: 
    # Large chunks (16k) for lightning fast ingestion to save Modal credits
    chunk_size = 16384
    overlap_size = 0
    ...
    # Calculate total chunks with overlap
    total_chunks = (len(text) + (chunk_size - overlap_size) - 1) // (chunk_size - overlap_size)
    
    with torch.no_grad():
        with tqdm(total=total_chunks, desc="  Processing Chunks", unit="chunk", leave=False) as pbar:
            # Overlap traversal: i moves by (chunk_size - overlap_size) each loop
            for i in range(0, len(text), chunk_size - overlap_size):
                chunk = text[i : i + chunk_size]
                # Increased max_length to 2048 to handle larger chunks efficiently
                encoded = system.tokenizer(chunk, return_tensors="pt", truncation=True, max_length=2048).to(system.device)
